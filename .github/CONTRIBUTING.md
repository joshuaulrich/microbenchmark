...
1. Create a *minimal* reproducible example. The process of creating a
   [*minimal*, reproducible example](http://stackoverflow.com/q/5963269/271616)
   ...
   issue.
1. Search current open GitHub [issues](https://github.com/joshuaulrich/microbenchmark/issues)
   to check if the bug/feature has already been reported/requested.
1. Ensure your fork and local copy are up-to-date, and verify the bug still
   exists in the HEAD of the master branch.
1. If the bug exists in the HEAD of master, and you can't find an open issue,
   then [open a new issue](https://github.com/joshuaulrich/microbenchmark/issues).
   Please be sure to:
...
1. Create a branch for the feature/bug fix reported in the issue. Please use a
...
1. A pull request should only be for one issue, so please `git rebase -i` and
   ...
   with `git push --force`.
1. Please write a great [commit message](#commit-messages).
1. It would be much appreciated if you also add tests that cover your changes.
...
1. The [Atom Contributing Guide](https://github.com/atom/atom/blob/master/CONTRIBUTING.md).
1. How to create a [minimal, reproducible example](http://stackoverflow.com/q/5963269/271616).
1. [How to Write a Git Commit Message](http://chris.beams.io/posts/git-commit/).
1. The [Mercurial Contributing Guide](https://www.mercurial-scm.org/wiki/ContributingChanges).
1. The [Hugo Contributing Guide](https://github.com/spf13/hugo/blob/master/CONTRIBUTING.md).
